-- Bản mở rộng đầy đủ của eduplan_workspace (bao trùm bản 20260823):
-- + Chuẩn hóa mã lớp khi học sinh tham gia (bỏ dấu cách, ký tự thừa, tiền tố LOP)
-- + Giới hạn số lần làm bài kiểm tra trực tuyến theo cấu hình của giáo viên,
--   đếm số lần làm (attemptCount) phía máy chủ
-- Chỉ cần chạy tệp này là đủ cho mọi thao tác workspace.
...
begin
  ...
  if p_action = 'join_class' then
    ...
    v_code := regexp_replace(upper(trim(coalesce(p_payload->>'code', ''))), '[^A-Z0-9]', '', 'g');
    v_code := regexp_replace(v_code, '^LOP', '');
    ...
    where regexp_replace(upper(coalesce(item->>'code', '')), '[^A-Z0-9]', '', 'g') = v_code
    limit 1;
    ...
    select coalesce(jsonb_agg(
      case when regexp_replace(upper(coalesce(item->>'code', '')), '[^A-Z0-9]', '', 'g') = v_code
  ...
  if p_action = 'submit_assessment' then
    ...
    -- Số lần cho phép: cấu hình attempts của đề; mặc định Luyện tập không giới hạn, còn lại 1 lần.
    v_attempts := coalesce(nullif(v_assessment->>'attempts', '')::int,
      case when coalesce(v_assessment->>'kind', '') = 'Luyện tập' then 0 else 1 end);
    select item into v_old_result
    from jsonb_array_elements(coalesce(v_data->'assessmentResults', '[]'::jsonb)) item
    where item->>'assessmentId' = v_assignment_id and item->>'studentKey' = v_actor.id::text
    limit 1;
    if v_attempts > 0 and coalesce(nullif(v_old_result->>'attemptCount', '')::int, case when v_old_result is null then 0 else 1 end) >= v_attempts then
      raise exception 'Em đã dùng hết số lần làm bài cho phép';
    end if;
    ...
    v_result := v_result || jsonb_build_object(
      ...
      'studentName', v_actor.name,
      'submittedAt', now(),
      'attemptCount', coalesce(nullif(v_old_result->>'attemptCount', '')::int, case when v_old_result is null then 0 else 1 end) + 1
    );
